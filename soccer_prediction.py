...
def split(data, test_proportion=0.3):
...
def build_model_logistic(target, data, acc=0.00000001, alpha=L1_ALPHA):
    ...
    data = data.copy()
    data['intercept'] = 1.0
    logit = sm.Logit(target, data, disp=True)
    return logit.fit_regularized(maxiter='defined_by_method',alpha=alpha, acc=acc, disp=False)
#    return logit.fit()    
